...
import argparse
import pandas as pd
from classes import Class_Value
...
def gcImgPath(tree):
    directory = "/content/tissue_pics/train/"  #set google colab directory
    original_path = find_file_val(tree, identifier="<path>")  #original path, on my personal computer
    head, tail = os.path.split(original_path)    #extracts tail (the file name)
    new_path = os.path.join(directory, tail)      #creates a new the new path for google colab
    return new_path
...
def construct_row(xml_filepath):
    ...
    #assemble the row
    row = []
    row.append(gcImgPath(tree))  #add the filepath to the row
