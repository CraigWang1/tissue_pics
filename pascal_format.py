...
def resize_and_save(voc, fnames):
    ...
    for i, fname in enumerate(fnames):
        ...
        if args.annot_dir:
            ...
            save_loc = os.path.join(voc, 'Annotations/{}.{}'.format(str(i), 'xml'))
            annot_loc = os.path.join(args.annot_dir, annot)
            new_bbox_xml(annot_loc, fname, small_fname, save_loc)
...
def new_bbox_xml(xmlfile, og_impath, new_impath, save_loc):
    xmin, ymin, xmax, ymax, name = adjust_bboxes(xmlfile, og_impath)
